// server/explorer-persistent.js - 支持数据持久化的区块链浏览器服务器
const express = require('express');
const cors = require('cors');
const bodyParser = require('body-parser');
const path = require('path');
const PersistentCosmosBlockchain = require('./blockchain-persistent');

const app = express();
const PORT = 3000;
const blockchain = new PersistentCosmosBlockchain();
...
app.post('/api/clear-data', async (req, res) => {
  try {
    await blockchain.clearData();
    res.json({ success: true, message: '持久化数据已清除' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.get('/api/data-info', (req, res) => {
  const fs = require('fs');
  const dataFile = path.join(__dirname, 'blockchain-data', 'blockchain.json');
  ...
  try {
    if (fs.existsSync(dataFile)) {
      ...
      res.json({
        ...
        usersCount: Object.keys(data.wallets || {}).length,
        validatorsCount: Object.keys(data.validators || {}).length
      });
    } else {
...
// 启动服务器
app.listen(PORT, () => {
  console.log(`🌐 持久化区块链浏览器运行在 http://localhost:${PORT}`);
});
